import { CommandInteraction, Message, MessageActionRow, MessageSelectMenu, MessageSelectOptionData, SelectMenuInteraction } from "discord.js"
...
async function securityChecks({ interaction, player }: {
  ...
  const cardInPlayerInventory = player.inventories.find((inventoryCard) => inventoryCard.id === cardInventoryId);
  ...
  if (cardInPlayerInventory.total < quantity) {
    await interaction.editReply(`Erreur, tu n'as pas assez de cartes (${cardInPlayerInventory.total} possédées)`);
    return null;
  }
  ...
  return {
    ...
    quantity,
    earningPoints: quantity * priceConfig[cardInPlayerInventory.type as CardRarity] * cardInPlayerInventory.cardType.level
  };
}
    ...
    .addComponents(
      new MessageSelectMenu()
        ...
        .addOptions(
          ...
          Array.from(Array(getHigherQuantity)).map((qty): MessageSelectOptionData => {
            return {
              label: String(qty + 1),
              value: String(qty + 1)
            }
