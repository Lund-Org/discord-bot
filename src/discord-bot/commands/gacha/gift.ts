...
export const gift = async (interaction: CommandInteraction) => {
  ...
  if (!foundGift) {
    return interaction.editReply('Le cadeau n\'existe pas ou tu n\'es pas dans sa période de validité');
  }
  ...
  if (unionCards.length) {
    const canvas = await generateDrawImage(interaction.user.username, unionCards);
  ...
  return attachment ? interaction.editReply({
    content: message,
    files: [attachment]
