import { CommandInteraction, Message, MessageActionRow, MessageEmbed, MessageSelectMenu, MessageSelectOptionData, SelectMenuInteraction } from "discord.js"
...
export const gold = async (interaction: SelectMenuInteraction) => {
  const player = await userNotFound({ interaction, relations: ['inventories', 'inventories.cardType'] })
  ...
  if (inventoryCardBasic && inventoryCardBasic.total >= 5) {
    ...
    return interaction.editReply('5 cartes basiques ont été transformée en une carte en or')
  } else if (inventoryCardBasic) {
    return interaction.editReply('Tu ne possèdes pas assez de cartes basic (5 cartes basiques = 1 carte en or)')
  } else {
