  ...
  return new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('option7')
        .setDescription('Choix 7')
        .setRequired(false)
    )
    .addStringOption((option) =>
      option.setName('option8')
        .setDescription('Choix 8')
        .setRequired(false)
    )
    .addStringOption((option) =>
      option.setName('option9')
        .setDescription('Choix 9')
        .setRequired(false)
    )
    .addStringOption((option) =>
      option.setName('option10')
        .setDescription('Choix 10')
        .setRequired(false)
    )
    .toJSON();
}
...
async function pollCallback(interaction: CommandInteraction) {
  ...
  const choices = Array.from(Array(10)).map(
    (n) => interaction.options.getString(`option${n + 1}`, n < 2 ? true : false)
  ).filter(Boolean)
  const emojis: Array<string> = choices.map((_, index): string => reactionEmojis[index])
