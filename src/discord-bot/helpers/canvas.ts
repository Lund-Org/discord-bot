...
function setDescriptionOfCard(
  ...
  for (let iteration = 10; fragmentedDescription.length && iteration > 0; --iteration) {
    let quit = false

    for (let wordsTaken = fragmentedDescription.length; wordsTaken > 0 && !quit; --wordsTaken) {
      const desc = fragmentedDescription.filter((_, index) => index < wordsTaken)

      if (ctx.measureText(desc.join(' ')).width < WIDTH_CARD - 2 * INNER_HORIZONTAL_MARGIN) {
        ctx.fillText(
          desc.join(' '),
          offsetX + INNER_HORIZONTAL_MARGIN,
          offsetY + CARD_HEADER + CARD_IMAGE_HEIGHT + CARD_TITLE_HEIGHT + (linesInput * 40)
        )
...
async function setStarsOfCard(
  ...
  try {
    ...
    for (let i = 0; i < numberOfStars; ++i) {
      ctx.drawImage(
        starImg,
        offsetX + (WIDTH_CARD / 2) - (numberOfStars * starImg.width / 2) + (i * starImg.width),
...
export const setupCardDrawBody = async (ctx: CanvasRenderingContext2D, cardList: CardDraw[]) => {
